...
def test_a_job_that_just_fits_is_tight_not_safe() -> None:
    est = cap.estimate(NOW)
    # Demand is worker * 2.0, so pick a worker size landing just inside available capacity.
    worker = est.safe_new_work_minutes / 2.0 * 0.95
    v = cap.pre_job_check(worker_minutes=worker, est=est, now=NOW)
    assert v.status == CapacityStatus.TIGHT.value
...
def test_a_multi_window_job_is_scheduled_across_windows_not_rejected() -> None:
    ...
    assert v.wait_until
    assert not v.needs_human
    assert "multi-window job, not a rejected one" in v.reason
